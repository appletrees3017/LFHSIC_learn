...
class IndpTest_LFMahalanobis(IndpTest):
    """
    ...
    def __init__(self, X, Y, device, alpha=0.05, n_permutation=100, null_gamma = True, split_ratio = 0.5):
        ...
        """
        super(IndpTest_LFMahalanobis, self).__init__(X, Y, alpha)
    ...
    def perform_test(self, rff_num, lr = 0.05, iter_steps=100, if_grid_search = False, debug = -1):
        ...
        wx, wy, weight_x, weight_y, _ = self.search_width_weight_fix(Xtr, Ytr, wx_init/2.0, wy_init/2.0, unit_rff_freqx_fix, unit_rff_freqy_fix, lr = lr, iter_steps = iter_steps, debug = debug) 
        ...
        if self.null_gamma == True:
            if self.device.type == "cuda":
                ...
                fX, fY = self.feat_gen(Xte, Yte, wx, wy, weight_x, weight_y)
            # fX, fY = self.feat_gen(Xte, Yte, wx, wy, weight_x, weight_y)
            ...
            res= self.cal_thresh(rfx, rfy, rfxc, rfyc)
            thresh=res[0]
        else:
            if self.device.type == "cuda":
                fX, fY = self.feat_gen(Xte, Yte, wx, wy, weight_x.cpu(), weight_y.cpu())
    ...
    def search_width_weight_fix(self, X, Y, wx_init, wy_init, unit_rff_freqx_fix, unit_rff_freqy_fix, lr = 0.05, delta_estimate_grad=1e-3, \
        ...
        for st in range(iter_steps):
            ...
            fX, fY = self.feat_gen(X, Y, wx, wy, weight_x, weight_y)
            rfx, rfy = self.rff_generate(fX, fY, unit_rff_freqx_fix, unit_rff_freqy_fix)
    ...
    def feat_gen(self, X, Y, wx, wy, weight_x, weight_y):
    ...
    def grid_search_init(self, X, Y, wx_mid, wy_mid, unit_rff_freqx_fix, unit_rff_freqy_fix):
        ...
        for facx in gwidth_factors:
            for facy in gwidth_factors:
                ...
                fX, fY = self.feat_gen(X, Y, wx/2.0, wy/2.0, weight_x, weight_y)
                rfx, rfy = self.rff_generate(fX, fY, unit_rff_freqx_fix, unit_rff_freqy_fix)
                rfxc = rfx - torch.mean(rfx,0)
                rfyc = rfy - torch.mean(rfy,0)
